...
export function BrandOverviewTab({ brand, outletId }: { brand: string; outletId: string }) {
  const { data, isLoading, isError } = useDashboard({ outletId, brand });
    ...
    <div className="space-y-6">
      ...
      {kpis.lowStockItems > 0 && (
        <div className="flex items-center gap-2 rounded-md border border-amber-300 bg-amber-50 px-4 py-2 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200">
          ...
          {kpis.lowStockItems} item{kpis.lowStockItems === 1 ? '' : 's'} running low on stock across {brand} outlets.
        </div>
      )}
